...
contract EIP7702HelperTest is Test, SessionKeyHelper {
    ...
    function test_hashAuthorization_notZero() public {
        Auth7702.Authorization memory auth = Auth7702.Authorization({
            chainId: block.chainid, logicContract: address(simpleLogic), nonce: 0
        });
    ...
    function test_hashAuthorization_componentOverload_matches() public {
        Auth7702.Authorization memory auth = Auth7702.Authorization({
            chainId: block.chainid, logicContract: address(simpleLogic), nonce: 1
        });
    ...
    function test_signAuthorization_recoversCorrectSigner() public {
        Auth7702.SignedAuthorization memory signed =
            signAuthorization(OWNER_PK, address(simpleLogic), 0, block.chainid);
    ...
    function test_signAuthorization_chainIdOverload_usesBlockChainId() public {
        Auth7702.SignedAuthorization memory signed =
            signAuthorization(OWNER_PK, address(simpleLogic), 0);
    ...
    function test_signExecutionRequest_712_recoversSessionKey() public {
        ...
        SignedExecutionRequest memory signed =
            signExecutionRequest712(SESSION_KEY_PK, req, domainSep);
    ...
    function test_buildBatchExecutionRequest_correctLength() public {
        ...
        calls[0] = SessionKeyHelper.Call({
            target: address(swapLogic),
            value: 0,
            data: abi.encodeCall(TokenSwapLogic.swap, (100e18))
        });
        calls[1] = SessionKeyHelper.Call({
            target: address(swapLogic),
            value: 0,
            data: abi.encodeCall(TokenSwapLogic.swap, (200e18))
        });
        calls[2] = SessionKeyHelper.Call({
            target: address(swapLogic),
            value: 0,
            data: abi.encodeCall(TokenSwapLogic.swap, (300e18))
        });

        ExecutionRequest memory req =
            buildBatchExecutionRequest(calls, 0, block.timestamp + 1 hours);
    ...
    function test_batchRequest_raw_recoversSessionKey() public {
        ...
        ExecutionRequest memory req =
            buildBatchExecutionRequest(calls, 1, block.timestamp + 1 hours);
        SignedExecutionRequest memory signed = signExecutionRequest(SESSION_KEY_PK, req);
    ...
    function testFuzz_hashAuthorization_deterministicForSameInputs(uint256 chainId, uint256 nonce)
        public
    {
